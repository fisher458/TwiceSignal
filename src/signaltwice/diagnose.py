...
from signaltwice.core.reader import ReadHandler
from signaltwice.core.writer import WriteHandler
from signaltwice.task.defaultParameter import ParameterHandler
from signaltwice.task.getBasicInfo import BasicInfoHandler
from signaltwice.task.reportSection import ReportSectionHandler
from signaltwice.task.visualizer import SmartPlotter, VisualizeConfig
...
class DiagnosticPipeline:
    ...
    def __init__(
        ...
        # Load configuration from defaultParameter (strategy registry).
        self.parameter_handler = ParameterHandler

        # Register core module entry points.
        self.reader = ReadHandler
        self.basic_info = BasicInfoHandler
        self.reporter = ReportSectionHandler
        self.writer = WriteHandler
        ...
        self.plotter = SmartPlotter(self.visual_config)
    ...
    def run_diagnosis(
        ...
        *,
        ...
        logger.info("Step start: Extract (read data) | file_path=%s", file_path)
        ...
        try:
            ...
            logger.info("Step end: Extract (read data)")
            mark_step("extract", True)
        except Exception as exc:
            logger.exception("Extract failed: read input file | file_path=%s", file_path)
        ...
        logger.info("Step start: Transform (preprocess data)")
        try:
            ...
            logger.info("Step end: Transform (preprocess data)")
            mark_step("preprocess", True)
        except Exception as exc:  # pragma: no cover - defensive safety net
            logger.exception("Transform failed: preprocess data")
        ...
        logger.info("Step start: Transform (compute diagnostics)")
        try:
            metrics = self.basic_info.execute_all(cleaned)
            if not isinstance(metrics, dict):
                raise ValueError("Diagnostics must return a dictionary of metrics.")
            logger.info("Step end: Transform (compute diagnostics)")
            mark_step("diagnostics", True)
        except Exception as exc:  # pragma: no cover - defensive safety net
            logger.exception("Transform failed: compute diagnostics")
        ...
        logger.info("Step start: Configure (derive default parameters)")
        default_params: Dict[str, Any] | None = None
        parameter_path: Path | None = None
        try:
            default_params = self.parameter_handler.produce_default_parameters(metrics)
            logger.info("Step end: Configure (derive default parameters)")

            # 4.1) Persist parameters (JSON).
            logger.info("Step start: Report (write parameters)")
            ...
            logger.info("Step end: Report (write parameters)")
            mark_step("parameters", True)
        except Exception as exc:
            logger.exception("Configure/Report failed: parameters")
        ...
        logger.info("Step start: Report (write report)")
        ...
        try:
            ...
            logger.info("Step end: Visualize (write plots)")
            mark_step("plots", True)
        except Exception as exc:
            logger.exception("Visualize failed: write plots")
    ...
    def _preprocess_eager(self, frame: pl.DataFrame) -> pl.DataFrame:
        ...
        logger.debug("Raw frame shape | rows=%s cols=%s", frame.height, frame.width)
        logger.debug("Raw columns | columns=%s", frame.columns)
        ...
        if all_null_cols:
            logger.debug("Dropping all-null columns | columns=%s", all_null_cols)
        ...
        logger.debug("Post-preprocess shape | rows=%s cols=%s", frame.height, frame.width)
        return frame

    def _preprocess_lazy(self, frame: pl.LazyFrame) -> pl.LazyFrame:
        columns = list(frame.schema.keys())
        if not columns:
            logger.debug("Empty schema detected | cols=0")
            return frame
        ...
        if row_count == 0:
            logger.debug("Empty frame detected | rows=0 cols=%s", len(columns))
            return frame
        ...
        logger.debug("Post-preprocess lazy schema | cols=%s", len(columns))
        return frame
